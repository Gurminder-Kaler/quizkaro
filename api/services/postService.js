...
const deletePostServiceFunc = async (req, res) => {
  ...
  console.log("asds");
  let post = await Post.findOne({ _id: req.body.postId }).lean().exec();
...
const getAllPostsViaUserIdServiceFunc = async (req, res) => {
  ...
  const user = await User.findOne(userFilter).lean().exec();
  if (!user) {
    res.json({
      status: 404,
      success: false,
  ...
  let allPosts = await Post.find(filter).lean().exec();
...
const getAllPostsServiceFunc = async (req, res) => {
  let allPosts = await Post.find().lean().exec();
